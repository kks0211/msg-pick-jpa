...
import {alertMessage, initMaxLength, serializeJSON} from '../common/utils';
...
const ShopRegister = (function () {
    ...
    const handlers = {
        ...
        onClickNextButton: async () => {
            try {
                ...
                Array.from(imageDropzone.files).map(file => formData.append('files', file));

                await registerShopApi(formData);


                //serializeJSON(shopRegisterFormEl);
                /*for (let file in imageDropzone.files) {
                    formData.append("file", file);
                }*/

                //Array.from(imageDropzone.files).map(file => formData.append('file[]', file));
            ...
            /*validator.validate()
                .then(async status => {
                    ...
                    try {
                        ...
                        const formData = new FormData(shopRegisterFormEl);
                        /!*for (let file in imageDropzone.files) {
                            formData.append("file", file);
                        }*!/
    ...
    const init = () => {
        ...
        imageDropzone = new Dropzone(shopImageDropzoneEl, {
            url: "false",
            paramName: "file",
            maxFiles: 10,
            maxFilesize: 10,
